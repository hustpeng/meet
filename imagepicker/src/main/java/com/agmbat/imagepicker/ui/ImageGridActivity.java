package com.agmbat.imagepicker.ui;
...
import com.agmbat.android.sysprovider.SysProvider;
import com.agmbat.imagepicker.DataHolder;
import com.agmbat.imagepicker.ImageDataSource;
import com.agmbat.imagepicker.ImagePicker;
import com.agmbat.imagepicker.R;
import com.agmbat.imagepicker.adapter.ImageFolderAdapter;
import com.agmbat.imagepicker.adapter.ImageRecyclerAdapter;
import com.agmbat.imagepicker.adapter.ImageRecyclerAdapter.OnImageItemClickListener;
import com.agmbat.imagepicker.bean.ImageFolder;
import com.agmbat.imagepicker.bean.ImageItem;
import com.agmbat.imagepicker.view.FolderPopUpWindow;
import com.agmbat.imagepicker.view.GridSpacingItemDecoration;
...
//        mImageGridAdapter.setOnImageItemClickListener(this);
        ...
        int spacing = (int) SysResources.dipToPixel(2);
    ...
    public void onImageItemClick(View view, ImageItem imageItem, int position) {
        //根据是否有相机按钮确定位置
        position = imagePicker.getPickerOption().isShowCamera() ? position - 1 : position;
        if (imagePicker.isMultiMode()) {
            Intent intent = new Intent(ImageGridActivity.this, ImagePreviewActivity.class);
...
//        mRecyclerAdapter.notifyItemChanged(position + (imagePicker.isShowCamera() ? 1 : 0));// 17/4/24  fix the position while click right bottom preview button
        for (int i = imagePicker.getPickerOption().isShowCamera() ? 1 : 0; i < mRecyclerAdapter.getItemCount(); i++) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data != null && data.getExtras() != null) {
            ...
            if (resultCode == RESULT_OK && requestCode == ImagePicker.REQUEST_CODE_TAKE) {
                //发送广播通知图片增加了
                SysProvider.scanMediaFile(this, imagePicker.getTakeImageFile());
