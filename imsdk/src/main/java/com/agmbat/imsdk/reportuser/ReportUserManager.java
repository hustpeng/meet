...
public class ReportUserManager {
     ...
     * 举报用户
     *
     * @param targetUid 被举报群
     * @param content
     * @param path
     * @param listener
     */
    public static void reportGroup(final String targetUid, final String content, final String path,
                                   final OnReportUserListener listener) {
        AsyncTaskUtils.executeAsyncTask(new AsyncTask<Void, Void, ApiResult>() {
            @Override
            protected ApiResult doInBackground(Void... voids) {
                ...
                ApiResult result = ReportUserApi.reportGroup(phone, ticket, targetUid, content, imageUrl);
                if (result == null) {
                    result = new ApiResult();
                    result.mResult = false;
                    result.mErrorMsg = "举报失败!";
