 ...
 * Builds and parses URI references which conform to <a href="http://www.faqs.org/rfcs/rfc2396.html">RFC 2396</a>.
 *
 * <p>
...
public abstract class Uri implements Comparable<Uri> {
    ...
    /*
     * 
     * This class aims to do as little up front work as possible. To accomplish that, we vary the implementation
     * depending on what the user passes in. For example, we have one implementation if the user passes in a URI string
     * (StringUri) and another if the user passes in the individual components (OpaqueUri).
     * 
     * Concurrency notes*: Like any truly immutable object, this class is safe for concurrent use. This class uses a
     * caching pattern in some places where it doesn't use volatile or synchronized. This is safe to do with ints
     * because getting or setting an int is atomic. It's safe to do with a String because the internal fields are final
     * and the memory model guarantees other threads won't see a partially initialized instance. We are not guaranteed
     * that some threads will immediately see changes from other threads on certain platforms, but we don't mind if
     * those threads reconstruct the cached result. As a result, we get thread safe caching with no concurrency
     * overhead, which means the most common case, access from a single thread, is as fast as possible.
     * 
     * From the Java Language spec.:
     * 
     * "17.5 Final Field Semantics
     * 
     * ... when the object is seen by another thread, that thread will always see the correctly constructed version of
     * that object's final fields. It will also see versions of any object or array referenced by those final fields
     * that are at least as up-to-date as the final fields are."
     * 
     * In that same vein, all non-transient fields within Uri implementations should be final and immutable so as to
     * ensure true immutability for clients even when they don't use proper concurrency control.
     * 
     * For reference, from RFC 2396:
     * 
     * "4.3. Parsing a URI Reference
     * 
     * A URI reference is typically parsed according to the four main components and fragment identifier in order to
     * determine what components are present and whether the reference is relative or absolute. The individual
     * components are then parsed for their subparts and, if not opaque, to verify their validity.
     * 
     * Although the BNF defines what is allowed in each component, it is ambiguous in terms of differentiating between
     * an authority component and a path component that begins with two slash characters. The greedy algorithm is used
     * for disambiguation: the left-most matching rule soaks up as much of the URI reference string as it is capable of
    ...
    /**
     * NOTE: EMPTY accesses this field during its own initialization, so this field *must* be initialized first, or else
     * EMPTY will see a null value!
     *
     * Placeholder for strings which haven't been cached. This enables us to cache null. We intentionally create a new
     * String instance so we can compare its identity and there is no chance we will confuse it with user data.
     */
    ...
    /**
     * Return a string representation of the URI that is safe to print to logs and other places where PII should be
     * avoided.
     * 
     * @hide
     */
    ...
    private static class StringUri extends AbstractHierarchicalUri {
        ...
        public Builder buildUpon() {
            if (isHierarchical()) {
                return new Builder().scheme(getScheme()).authority(getAuthorityPart()).path(getPathPart())
                        .query(getQueryPart()).fragment(getFragmentPart());
            } else {
    ...
    /**
     * Creates an opaque Uri from the given components. Encodes the ssp which means this method cannot be used to create
     * hierarchical URIs.
     *
    ...
    private abstract static class AbstractHierarchicalUri extends Uri {
        ...
        private String parseHost() {
            ...
            String encodedHost =
                    portSeparator == NOT_FOUND ? authority.substring(userInfoSeparator + 1) : authority.substring(
     ...
     * {@code <relative or absolute path>?<query>#<fragment>} or {@code //<authority><absolute path>?<query>#<fragment>}
     *
     * <p>
     * An opaque URI follows this pattern: {@code <scheme>:<opaque part>#<fragment>}
     *
     * <p>
     * Use {@link Uri#buildUpon()} to obtain a builder representing an existing URI.
     */
    ...
    /**
     * Returns a set of the unique names of all query parameters. Iterating over the set will return the names in order
     ...
     *
     * <p>
     * <strong>Warning:</strong> Prior to Ice Cream Sandwich, this decoded the '+' character as '+' rather than ' '.
     *
     * @param key which will be encoded
     * @throws UnsupportedOperationException if this isn't a hierarchical URI
     * @throws NullPointerException if key is null
     ...
     * and "0" are interpreted as <code>false</code>, everything else is interpreted as <code>true</code>.
     *
     ...
     *
     * <p class="note">
     * This method does <em>not</em> validate bad URI's, or 'fix' poorly formatted URI's - so do not use it for input
     * validation. A Uri will always be returned, even if the Uri is badly formatted to begin with and a scheme
    ...
    /**
     * Encodes characters in the given string as '%'-escaped octets using the UTF-8 scheme. Leaves letters ("A-Z",
     * "a-z"), numbers ("0-9"), and unreserved characters ("_-!.~'()*") intact. Encodes all other characters with the
