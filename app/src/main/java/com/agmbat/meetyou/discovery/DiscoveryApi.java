...
public class DiscoveryApi {
     ...
     * ticket	YES	String	The auth ticket
     * center	Yes	String	搜索的中心点，一般是当前用户的地理位置，格式为<纬度,经度>，例如”30.5,111.2”
     * hobby	Yes	String	兴趣关键字,例如”羽毛球”, 每次只能搜索一个兴趣关键字
     * pageindex	No	Integer	分页获取，第几页。zero based，第一页是0，第二页是1，…
     * 如果不提供，默认为0。
     * sign	Yes	String	API调用签名
     * <p>
     * 返回结果优先按照distance排序
     ...
     * @return
     */
    public static DiscoveryApiResult getHobby(String uid, String ticket, String hobby, int pageIndex) {
        String apiName = "nearby";
        String url = Api.getBaseDiscoveryUrl(apiName);
        HttpRequester.Builder builder = new HttpRequester.Builder();
        builder.baseUrl(url);
     ...
     * ticket	YES	String	The auth ticket
     * center	Yes	String	搜索的中心点，一般是当前用户的地理位置，格式为<纬度,经度>，例如”30.5,111.2”
     * birthplace	Yes	String	籍贯。例如”””湖北省武汉市”
     * pageindex	YES	Integer	分页获取，第几页。zero based，第一页是0，第二页是1,如果不提供，默认为0。
     * sign	YES	String	API调用签名
     * <p>
     * 返回内容如下：
     * <p>
     ...
     * @return
     */
    public static DiscoveryApiResult getBirthplace(String uid, String ticket, String birthplace, int pageIndex) {
        String apiName = "nearby";
        String url = Api.getBaseDiscoveryUrl(apiName);
        HttpRequester.Builder builder = new HttpRequester.Builder();
        builder.baseUrl(url);
