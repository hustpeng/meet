    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
                ...
                checkTaskNotActual();
                bitmap = decodeImage(Scheme.wrapUri("file", imageFile.getAbsolutePath()));
            }
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                ...
                if (options.isCacheOnDisk() && tryCacheImageOnDisk()) {
                    ...
                    if (imageFile != null) {
                        imageUriForDecoding = Scheme.wrapUri("file", imageFile.getAbsolutePath());
                    }
                }
    ...
    private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
        ...
        if (targetFile != null && targetFile.exists()) {
            ...
            ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey,
                    Scheme.wrapUri("file", targetFile.getAbsolutePath()), uri, targetImageSize, ViewScaleType.FIT_INSIDE,
